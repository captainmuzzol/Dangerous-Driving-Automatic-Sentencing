...
def place_fact(fact):
    ...
    if fact.find("行驶至") != -1 and (("路口" > "行驶至") or ("路段" > "行驶至")):
        start = "行驶至"
    i = 0
    if start and end in fact:
        if start > end:
            # print('ExtraDef调试信息fact:', fact)
            # print('ExtraDef调试信息start:', start)
            # print('ExtraDef调试信息end:', end)
            # print('ExtraDef调试信息fact.index(start):', fact.index('行驶至'))
            # print('ExtraDef调试信息fact.index(end):', fact.index(end))
            for i in range(fact.index(start), fact.index(end)):
                # print('ExtraDef调试信息:', fact[i])
                fact_place = fact_place + fact[i]
                i += 1
                # print('ExtraDef调试信息:', fact_place)
        else:
